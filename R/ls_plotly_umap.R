...
#' @param type type of plotly chart to render, usually 'scattergl' for performance
#' @param group_var Which variable to colour by (as a string)
...
ls_plotly_umap <- function(data, x, y, type, group_var, key, text_var,
  ...
  data %>%
    ...
    plotly::layout(
      ...
      yaxis = axis_config,
      newshape = list(
        fillcolor = "#ff5718", # Colour for shapes
        linecolor = "black",
        opacity = 0.2
      )
    ) %>%
