...
conversation_landscape <- function(data, ..., id = document, text_var = text, colour_var, cleaned_text_var = clean_text, date_var = date, sentiment_var = sentiment, url_var = permalink, size = 2, x_var = V1, y_var = V2, type = "scattergl", colour_mapping = NULL) {
  ...
  # hide UI ----
  ui <- shiny::navbarPage("Conversation Landscape",
    theme = shinythemes::shinytheme("cosmo"), position = "fixed-top",
    ...
    #---- Distribution Tab ----
    shiny::tabPanel(
      "Distribution Plots", shiny::fluidPage(
    ...
    #---- Bigram Tab ----
    shiny::tabPanel(
      "Bigram Network", shiny::fluidPage(
        theme = shinythemes::shinytheme("cosmo"),
        gotop::use_gotop()
      ),
      shiny::br(),
  ...
  server <- function(input, output, session) {
    ...
    output$umapPlot <- plotly::renderPlotly({
      reactive_data() %>%
        dplyr::rename(
          V1 = {{ x_var }}, # rename these variables to avoid problems with plotly NSE and not have to rename the eventual output's columns.
          V2 = {{ y_var }},
          id_var = {{ id }},
          text_var = {{ text_var }},
    ...
    output$highlightedTable <- DT::renderDataTable({
      ...
      DT::datatable(df,
        filter = "top", options = list(
          pageLength = 25,
          dom = '<"top" ifp> rt<"bottom"lp>', autoWidth = FALSE
        ), # TODO check adding l worked
        style = "bootstrap", rownames = FALSE,
        escape = FALSE
      ) # Add escape = False to ensure the HTML clicks work properly
    })
